var _toConsumableArray;module.import("babel-runtime/helpers/toConsumableArray",{"default":function(v){_toConsumableArray=v}});var _extends;module.import("babel-runtime/helpers/extends",{"default":function(v){_extends=v}});var _typeof;module.import("babel-runtime/helpers/typeof",{"default":function(v){_typeof=v}});
...
var useParsedSourceMap = function useParsedSourceMap(pathForSourceMap) {
...
// cwd.
var findUpwards = function findUpwards(predicate, startPath) {
...
files.findAppDir = function (filepath) {
  var isAppDir = function isAppDir(filepath) {
    ...
    try {
      // use try/catch to avoid the additional syscall to files.exists
      return files.stat(files.pathJoin(filepath, '.meteor', 'packages')).isFile();
    } catch (e) {
      return false;
    }
  };
...
files.findPackageDir = function (filepath) {
  var isPackageDir = function isPackageDir(filepath) {
    try {
...
files.getToolsVersion = function () {
  if (!files.inCheckout()) {
    var isopackJsonPath = files.pathJoin(files.getCurrentToolsDir(), '..', // get out of tool, back to package
    'isopack.json');
    ...
    var unipackageJsonPath = files.pathJoin(files.getCurrentToolsDir(), '..', // get out of tool, back to package
    'unipackage.json');
...
files.getSettings = function (filename, watchSet) {
  ...
  if (buffer === null) {
    buildmessage.error("file not found (settings file)", { file: filename });
    return null;
  }

  if (buffer.length > 0x10000) {
    buildmessage.error("settings file is too large (must be less than 64k)", { file: filename });
    return null;
  }
  ...
  if (str.match(/\S/)) {
    try {
      ...
      buildmessage.error("parse error reading settings file", { file: filename });
    }
  }
...
files.rm_recursive = Profile("files.rm_recursive", function (p) {
  if (Fiber.current && Fiber["yield"] && !Fiber["yield"].disallowed) {
    new Promise(function (resolve, reject) {
      rimraf(files.convertToOSPath(p), function (err) {
...
var makeTreeReadOnly = function makeTreeReadOnly(p) {
  try {
  ...
  if (stat.isFile()) {
    var permissions = stat.mode & 511;
    var readOnlyPermissions = permissions & 365;
...
files.treeHash = function (root, options) {
  options = _.extend({
    ignore: function ignore(relativePath) {
      return false;
    }
  }, options);
  ...
  var updateHash = function updateHash(text) {
    hashLog && hashLog.push(text);
    hash.update(text);
  };

  var traverse = function traverse(relativePath) {
    ...
    if (stat.isDirectory()) {
      ...
      updateHash('file ' + JSON.stringify(relativePath) + ' ' + stat.size + ' ' + files.fileHash(absPath) + '\n');
      if (stat.mode & 64) {
        updateHash('exec\n');
      }
      ...
      updateHash('symlink ' + JSON.stringify(relativePath) + ' ' + JSON.stringify(files.readlink(absPath)) + '\n');
    }
...
files.mkdir_p = function (dir, mode) {
  ...
  var pathIsDirectory = function pathIsDirectory(path) {
...
files.cp_r = function (from, to, options) {
  ...
  _.each(files.readdir(from), function (f) {
    ...
    var stats = options.preserveSymlinks ? files.lstat(fullFrom) : files.stat(fullFrom);
    if (stats.isDirectory()) {
      ...
      var mode = stats.mode & 64 ? 511 : 438;
...
files.copyFile = function (from, to) {
  var origMode = arguments.length <= 2 || arguments[2] === undefined ? null : arguments[2];

  files.mkdir_p(files.pathDirname(files.pathResolve(to)), 493);
  ...
  // into a writable app.
  var mode = origMode & 64 ? 511 : 438;

  copyFileHelper(from, to, mode);
};
files.copyFile = Profile("files.copyFile", files.copyFile);

var copyFileHelper = function copyFileHelper(from, to, mode) {
...
files.mkdtemp = function (prefix) {
  var make = function make() {
    ...
    var tries = 3;
    while (tries > 0) {
      var dirPath = files.pathJoin(tmpDir, prefix + (Math.random() * 0x100000000 + 1).toString(36));
      try {
        files.mkdir(dirPath, 448);
        return dirPath;
      } catch (err) {
        tries--;
      }
  ...
  var fileStream = fstream.Reader({
    ...
    filter: function filter(entry) {
      ...
      if (entry.type === "Directory") {
        entry.mode = (entry.mode || entry.props.mode) | 320;
...
files.renameDirAlmostAtomically = function (fromDir, toDir) {
  ...
  try {
    ...
    mv(toDir, garbageDir,  function(err) {
    if (err) { throw err; }
  })
...
files.renameDirAlmostAtomically = Profile("files.renameDirAlmostAtomically", files.renameDirAlmostAtomically);

files.writeFileAtomically = function (filename, contents) {
  var tmpFile = files.pathJoin(files.pathDirname(filename), '.' + files.pathBasename(filename) + '.' + utils.randomToken());
  files.writeFile(tmpFile, contents);
  //patch by f0x
  //files.rename(tmpFile, filename);
  mv(tmpFile, filename,  function(err) {
    if (err) { throw err; }
  })
};
files.writeFileAtomically = Profile("files.writeFileAtomically", files.writeFileAtomically);
...
files.symlinkOverSync = function (linkText, file) {
  fiberHelpers.noYieldsAllowed(function () {
    ...
    var tmpSymlink = files.pathJoin(files.pathDirname(file), "." + files.pathBasename(file) + ".tmp" + utils.randomToken());
...
files.runJavaScript = function (code, options) {
  ...
  return Profile.time('runJavaScript ' + filename, function () {
    ...
    if (options.sourceMap) {
      ...
      chunks.push(sourcemap.SourceNode.fromStringWithSourceMap(code, consumer));
    } else {
      chunks.push(code);
    }
    ...
    if (options.sourceMap) {
      ...
      if (options.sourceMapRoot) {
        ...
        parsedSourceMap.sourceRoot = files.pathJoin(options.sourceMapRoot, parsedSourceMap.sourceRoot || '');
      }
    ...
    try {
      ...
      try {
        ...
        if (_typeof(parseError.loc) !== "object") {
          throw parseError;
        }
...
var getLines = function getLines(file) {
...
files.getHomeDir = function () {
  if (process.platform === "win32") {
    return files.pathDirname(files.convertToStandardPath(process.env.METEOR_INSTALLATION));
  } else {
    return process.env.HOME;
  }
};

files.currentEnvWithPathsAdded = function () {
  var env = _extends({}, process.env);

  var pathPropertyName = void 0;
  if (process.platform === "win32") {
    ...
    pathPropertyName = _.find(Object.keys(env), function (key) {
  ...
  var convertedPaths = paths.map(function (path) {
    return files.convertToOSPath(path);
  });
  var pathDecomposed = (env[pathPropertyName] || "").split(files.pathOsDelimiter);
  pathDecomposed.unshift.apply(pathDecomposed, _toConsumableArray(convertedPaths));
...
var ensureBatExtension = function ensureBatExtension(p) {
...
files._generateScriptLinkToMeteorScript = function (scriptLocation) {
  ...
  var scriptLocationConverted = scriptLocationIsAbsolutePath ? files.convertToWindowsPath(scriptLocation) : "%~dp0\\" + files.convertToWindowsPath(scriptLocation);

  var newScript = ["@echo off", "SETLOCAL", "SET METEOR_INSTALLATION=%~dp0%",
  ...
  "\"" + scriptLocationConverted + "\" %*", "ENDLOCAL",
...
files._getLocationFromScriptLinkToMeteorScript = function (script) {
  ...
  var scriptLocation = _.last(lines).replace(/^rem /g, '');
  var isAbsolute = true;
...
function wrapFsFunc(fsFuncName, pathArgIndices, options) {
  ...
  function makeWrapper(_ref) {
    var alwaysSync = _ref.alwaysSync;
    var sync = _ref.sync;
    ...
    function wrapper() {
      ...
      for (var j = pathArgIndices.length - 1; j >= 0; --j) {
        var i = pathArgIndices[j];
        args[i] = files.convertToOSPath(args[i]);
      }

      var canYield = Fiber.current && Fiber["yield"] && !Fiber["yield"].disallowed;
      var shouldBeSync = alwaysSync || sync;
      ...
      // these calls sync.
      var isQuickie = fsFuncName === 'stat' || fsFuncName === 'rename';
      ...
      if (canYield && shouldBeSync && !isQuickie) {
        ...
        return options.modifyReturnValue ? options.modifyReturnValue(result) : result;
      } else if (shouldBeSync) {
        // Should be sync but can't yield: we are not in a Fiber.
        // Run the sync version of the fs.* method.
        var _result = fsFuncSync.apply(fs, args);
        return options.modifyReturnValue ? options.modifyReturnValue(_result) : _result;
      } else if (!sync) {
        var _ret = function () {
  ...
  files.fsFixPath[fsFuncName] = Profile('wrapped.fs.' + fsFuncName, makeWrapper({ sync: false }));
  files.fsFixPath[fsFuncName + 'Sync'] = Profile('wrapped.fs.' + fsFuncName + 'Sync', makeWrapper({ sync: true }));
}

wrapFsFunc("writeFile", [0]);
wrapFsFunc("appendFile", [0]);
wrapFsFunc("readFile", [0], {
  modifyReturnValue: function modifyReturnValue(fileData) {
...
function enableCache(name) {
  ...
  files[name] = function () {
    ...
    var result = method.apply(files, args);
...
files.exists = files.existsSync = function (path, callback) {
...
wrapFsFunc("readdir", [0], {
  modifyReturnValue: function modifyReturnValue(entries) {
...
files.pathwatcherWatch = function () {
  ...
  return (_require = require("pathwatcher")).watch.apply(_require, args);
};
...
files.readBufferWithLengthAndOffset = function (filename, length, offset) {
  ...
  if (length > 0) {
    ...
    try {
      var count = files.read(fd, data, 0, length, offset);
    } finally {
      files.close(fd);
    }
    if (count !== length) {
